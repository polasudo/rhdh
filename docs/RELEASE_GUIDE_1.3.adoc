 # Release Guide for RHDH 1.3 and earlier (DEPRECATED)

== Summary of Release Process

The table below provides a high-level overview of the release process before diving into detailed steps:

[options="header"]
|===
| Step | Key Action | Responsible Team
| 1. Code Freeze | Block changes & freeze pipeline | Release Engineering
| 2. RC Build | Push to Quay & Errata | Release Engineering
| 3. Testing | Smoke tests & QE validation | QE Team
| 4. GA Week Prep | Update Docs & obtain approvals from stakeholders | QE, Security Team, Docs
| 5. GA Release | Re-render & publish FBCs; push Helm chart; tag repos | Release Engineering
| 6. Post-Release | Remove EOL versions & security checks | Release Engineering
|===

See also link:https://gitlab.cee.redhat.com/rhidp/rhdh/-/blob/rhdh-1-rhel-9/docs/RELEASE_GUIDE_1.3.adoc#user-content-summary-of-release-tasks[Summary of Release tasks]
...
====
NOTE: As of 1.3.5, we've been asked to use RHSA instead of RHBA when creating erratas. HOWEVER, a **Product Security Approval Request** IS NOT REQUIRED. Moreover Errata System has updated its authentication method to meet approved Red Hat standards.  At this time, users are no longer allowed to authenticate using kerberos ID and password but rather must use kinit for authentication.

If you encounter an "Unauthorized" error when accessing the Errata System, and you know you have an Errata System account make use of the https://www.ibm.com/docs/pl/aix/7.2?topic=k-kinit-command[kinit command]:

Run https://www.ibm.com/docs/pl/aix/7.2?topic=k-klist-command[klist] in your terminal to check for a valid ticket. If no valid ticket is found, run: `kinit <your_username>` and enter your password. Then verify again with klist.

Test Kerberos Authentication by running `curl --negotiate -u : <https://errata.engineering.redhat.com/kerberos_test`

A successful response should display: Connected with kerberos user <your_username>@IPA.REDHAT.COM

Launch the browser with: `google-chrome --auth-server-whitelist="errata.engineering.redhat.com"` or refresh the existing tab.
====
...
### Trigger respins, 1.3 and earlier (OSBS)
...
For 1.3+, run the link:../build/scripts/renderCatalogs.sh[renderCatalogs] script:

```
./build/scripts/renderCatalogs.sh # see usage instructions 
```
...
.. helm chart from https://github.com/rhdh-bot/openshift-helm-charts/tree/rhdh-1.3-rhel-9/installation#installation
.. operator install from https://github.com/redhat-developer/rhdh-operator/blob/main/.rhdh/scripts/install-rhdh-catalog-source.sh
+
[WARNING]
====
If the changes you expect are not seen when installing via Operator, but are seen via Helm, re-render the FBCs again and rebuild in Konflux.
====
+
...
./copyIIBsToQuay.sh -p -t ${TAG} ${EXTRA_TAG}
```
+
** verify the IIB is correct:
+
```
./checkImagesInIIB.sh -y -q "quay.io/rhdh/iib:${STREAM}-v4.14-x86_64" --bundlefilter "v${STREAM}" && \
...
* if this RC build includes the same fixes as any link:https://errata.engineering.redhat.com/advisory/filters/4213[pending Freshmaker erratas], you should send an email to 
secalert@redhat.com requesting they drop unneeded/outadated link:https://errata.engineering.redhat.com/advisory/filters/4213[erratas]. 
This will prevent unexpected releases from landing in the link:https://catalog.redhat.com/search?gs&q=rhdh&searchType=containers[Red Hat Ecosystem Catalog].
...
. [ ] Re-render the FBC catalogs with link:https://gitlab.cee.redhat.com/rhidp/rhdh/-/blob/rhdh-1.3-rhel-9/build/scripts/renderCatalogs.sh#L66[`renderCatalogs.sh --rhec`], to switch to registry.redhat.io/rhdh instead of quay.io/rhdh image references. This should take about 2 mins per OCP version, and the builds in Konflux should take 5-10 mins total. Whole step should be under 15 mins. 
+
```
...
git checkout rhdh-1.3-rhel-9
RHDH_VERSION="1.3.5"
alias cp=cp
for OCP_VERSION in 4.14 4.15 4.16 4.17 4.18; do \
  time ./build/scripts/renderCatalogs.sh --rhec --versions "${OCP_VERSION}" -v "${RHDH_VERSION}" \
...
# create pull request
./build/helm/prepare.sh --chart-version 1.3.5 --rhdh-version 1.3-142 --chart-branch release-1.3 --catalog git@github.com:rhdh-bot/openshift-helm-charts.git --publish
```
+
You should see something like this:
+
```
Create chart for 1.3-142 (1.3.5)
...
...
Chart version:        1.3.5
Developer Hub image:  registry.redhat.io/rhdh/rhdh-hub-rhel9:1.3-142
Branch:               https://github.com/rhdh-bot/openshift-helm-charts/tree/redhat-developer-hub-1.3.5
...
[release-1.3.5 daf0844] chore: chart: add Red Hat Developer 1.3.5 for registry.redhat.io/rhdh/rhdh-hub-rhel9:1.3-142
...
...
. [ ] Find the latest operator bundle tag from `registry.redhat.io/rhdh/rhdh-operator-bundle:1.3`
+
```
1.3-137
```
...
./build/scripts/kfuxRelease.sh --prod   --fbc :1.3-137 -v 1.3.5 --debug
```
...
./build/scripts/copyIIBsToQuay.sh -t 1.3-137 --force --push -e 1.3.5.GA
```
...
./build/scripts/tagRelease.sh -v 1.3.5 -t 1.3 -gh release-1.3 -pd rhdh-1.3-rhel-9 -ghtoken $GITHUB_TOKEN
...
... [ ] switch to `fast-1.3` or `fast` subscription channel
... [ ] verify update from of 1.3.4 -> 1.3.5 
+
...
... [ ] upgrade to a newer chart - see link:https://github.com/rhdh-bot/openshift-helm-charts/tree/rhdh-1.3-rhel-9/installation[installation]
+
...
Due to a known issue with Pyxis not being able to update from Errata after onbooarding to Konflux, your pushed containers in registry.redhat.io will NOT be listed in catalog.redhat.com.

See https://gitlab.cee.redhat.com/konflux/docs/users/-/merge_requests/219#note_14312475 and https://issues.redhat.com/browse/CWFHEALTH-3992 for more details
...
- send an email to secalert@redhat.com requesting they drop any link:https://errata.engineering.redhat.com/advisory/filters/4213[stalled or outdated erratas created by Freshmaker].
