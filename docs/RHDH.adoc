...
====
NOTE: There currently is no https://github.com/janus-idp/operator/tree/main/bundle[upstream operator-bundle]. link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/tree/rhdh-1.0-rhel-9/distgit/containers/rhdh-operator-bundle[See midstream] and `(TODO)` https://issues.redhat.com/browse/RHIDP-488
====
...
. https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/tree/rhdh-1.0-rhel-9/ (midstream - `(TODO)` rename this repo - https://issues.redhat.com/browse/RHIDP-361)
...
.. https://pkgs.devel.redhat.com/cgit/containers/rhdh-hub/tree/?h=rhdh-1.0-rhel-9 
.. https://pkgs.devel.redhat.com/cgit/containers/rhdh-operator/tree/?h=rhdh-1.0-rhel-9 
.. https://pkgs.devel.redhat.com/cgit/containers/rhdh-operator-bundle/tree/?h=rhdh-1.0-rhel-9 
...
* link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub[OSBS midstream configuration for container builds] - includes link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/tree/rhdh-1.0-rhel-9/distgit/containers/rhdh-hub[Dockerfile.in template], link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/blob/rhdh-1.0-rhel-9/upstream_sources.yml[upstream_sources.yml], and yet another place to set link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/blob/rhdh-1.0-rhel-9/gcp_env/product-version[version info]. Note that branch must be the same as the downstream repo, eg., `rhdh-1.0-rhel-9` (not `main`)
...
To test a local change to the Dockerfile in Brew/OSBS without committing it first, you can either submit a link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/merge_requests[MR] against the midstream repo's `.in` files in link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/tree/rhdh-1.0-rhel-9/distgit/containers/rhdh-hub[distgit/containers/rhdh-hub], then wait for Jenkins to build that MR and generate a container downstream in Brew, or you can:

. First, get `devel` permission and install `brew` and `rhpkg` -- see link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/blob/rhdh-1.0-rhel-9/docs/PERMISSIONS.adoc#user-content-devel-group[PERMISSIONS.adoc].
...
```
git clone ssh://<kerberosUser>@pkgs.devel.redhat.com/containers/rhdh-hub && cd rhdh-hub && git checkout rhdh-1.0-rhel-9
...
rhpkg container-build --target rhdh-1.0-rhel-9-containers-candidate --scratch
...
* https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/pipelines 
* https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/pipeline_schedules
...
Schedule: https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/pipeline_schedules
...
. Check for recent changes in https://github.com/janus-idp/backstage-showcase/commits/1.0 (or main branch) since the last build have been synced to midstream as a new MR, eg., link:https://github.com/janus-idp/backstage-showcase/pull/305[PR 305] -> link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/merge_requests/33[MR 33]:
...
* NOTE: there is no upstream operator-bundle. link:https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/tree/rhdh-1.0-rhel-9/distgit/containers/rhdh-operator-bundle[See midstream].
