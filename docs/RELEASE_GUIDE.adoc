...
. After link:../build/scripts/tagRelease.sh[tagRelease.sh] runs, 7 GitHub branches(`release-1.y`) and 2 GitLab branches (`rhdh-1.y-rhel-9`) are created/updated.
...
.. link:https://github.com/redhat-ai-dev/rhdh-rag-content/branches/all?query=1.&lastTab=overview[https://github.com/redhat-ai-dev/rhdh-rag-content]
.. link:https://github.com/redhat-ai-dev/lightspeed-configs/branches/all?query=1.&lastTab=overview[https://github.com/redhat-ai-dev/lightspeed-configs]
...
== Create new GL pipelines

Go to link:https://gitlab.cee.redhat.com/rhidp/rhdh/-/pipeline_schedules[rhdh pipeline schedules] and create (or repurpose) a new pipeline for the new `1.y` branch, then point it at the recently created `rhdh-1.y-rhel-9` branch. 
...
Repeat for link:https://gitlab.cee.redhat.com/rhidp/rhdh-plugin-catalog/-/pipeline_schedules[rhdh-plugin-catalog pipeline schedules].
...
=== Plugin Catalog and Index

Before even building the hub and chart, or the bundle and FBCs, you need to get an updated set of plugins and an index image pushed to production.

This is because currently, the chart and bundle refer to the index image by digest, and that can't be computed in advance. Pushing to stage is also not currently a viable option.

So, if this is a .z RC, make sure you have updated the KRD repo with teh latest plugin tags:

https://gitlab.cee.redhat.com/releng/konflux-release-data/-/merge_requests/17574/diffs

Then you can use that information to push the updated plugins to production, and then the index.

For 1.9.4, the plugins are:

* registry.redhat.io/rhdh/red-hat-developer-hub-backstage-plugin-global-header:1.9.4--1.20.1
* registry.redhat.io/rhdh/red-hat-developer-hub-backstage-plugin-orchestrator:1.9.4--5.4.7
* registry.redhat.io/rhdh/red-hat-developer-hub-backstage-plugin-orchestrator-backend:1.9.4--8.6.7
* registry.redhat.io/rhdh/red-hat-developer-hub-backstage-plugin-orchestrator-backend-module-loki:1.9.4--1.0.6
* registry.redhat.io/rhdh/red-hat-developer-hub-backstage-plugin-orchestrator-form-widgets:1.9.4--1.6.9
* registry.redhat.io/rhdh/red-hat-developer-hub-backstage-plugin-scaffolder-backend-module-orchestrator:1.9.4--1.3.6

Once merged, you can run plugin and index releases with `./build/scripts/kfuxReleasse.sh`

Once released, continue below.
...
# if this is the first time checking out a branch for the rhidp/rhdh repo, initialize submodules
...
As of 1.9, RHDH consists of its own chart, plus the link:https://github.com/redhat-developer/rhdh-chart/tree/main/charts/orchestrator-infra[Orchestrator Infra Helm Chart],which is a standalone chart that deploys the required OpenShift infrastructure to support the Orchestrator flavor of RHDH. Specifically, it installs the OpenShift Serverless Operator and Serverless Logic Operator, and applies required Knative CRDs. These components are necessary for enabling orchestration features in Red Hat Developer Hub.
...
+
. link:../build/helm/prepare.sh[Push the helm charts].
+
----
# log in as the bot
...
# create pull requests for RHDH and Orchestrator charts
branch=release-1.y
version=1.y.z
./build/helm/prepare.sh --chart-version $version --rhdh-version $version --chart-branch $branch --publish && \
./build/helm/prepare.sh --chart-version $version --rhdh-version $version --chart-branch $branch --publish \
...
----
+
If you generated the PRs using the rhdh-bot user's GH token, they should be merged automatically once the chart PR tests pass.
...
2. Run link:../build/scripts/updateBaseImages.sh[updateBaseImages.sh] against the upstream repos link:https://github.com/redhat-developer/rhdh/[rhdh], link:https://github.com/redhat-developer/rhdh-operator/[rhdh-operator], and link:https://github.com/redhat-ai-dev/rhdh-rag-content/[rhdh-rag-content] to update base images in containerfiles.
