...
If so, be sure to add a new application, component, RP + RPA. If removing support, remove RP and RPAs, but keep the application and component until the older release is EOL. 
...
. [ ] Verify the latest rhdh-hub tag you want to release exists at https://catalog.redhat.com/software/containers/rhdh/rhdh-hub-rhel9/645bd4c15c00598369c31aba?image=68a46041e06f8cfcc4f7904e, eg., `1.7.0`
+
...
./build/helm/prepare.sh --chart-version 1.7.0 --rhdh-version 1.7.0 --chart-branch release-1.7 --publish 

Create chart for 1.7.0 (1.7.0)
...
Create chart for 1.7.0 (1.7.0)
[INFO] Set Orchestrator Infra chart version to 1.7.0
...
...
./build/scripts/kfuxRelease.sh --fbc :1.7.0 --prod --debug [--auto]
```
...
./build/scripts/tagRelease.sh -v 1.7.0 -t 1.7 -gh release-1.7 --midstream-branch rhdh-1.7-rhel-9 --clean --force-update --nobuild
```
