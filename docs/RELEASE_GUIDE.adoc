...
Once set up, you can then run the branch creation script whenever the cats are done playing in the litter box and it's time for feature or code freeze. 
...
. Request new Brew Targets & product variant (PV) be created via link:https://issues.redhat.com/browse/SPMM-16740[SPMM ticket like SPMM-16740] and by pinging @ahills / @siege on slack
...
### Create branches (eg., 1.2.x, 1.3.x)

Use the link:../build/scripts/tagRelease.sh[tagRelease.sh] script like this:
```
# to create 1.2.x + rhdh-1.2-rhel-9 branches, then update main + rhdh-1-rhel-9 branches
export GITHUB_TOKEN=<your_GH_token>
./build/scripts/tagRelease.sh -t 1.2 --branchfrom main -gh 1.2.x
```
...
NOTE that downstream branch creation is still a manual step: 
...
. Create new gitlab pipeline schedule for stable branch 1.2 (to run in parallel with CI builds from 1.x)
...
. You can also force a respin using this script: https://gitlab.cee.redhat.com/rhidp/rhdh/-/blob/rhdh-1-rhel-9/build/scripts/triggerRespin.sh[build/scripts/triggerRespin.sh]
...
```
./copyIIBsToQuay.sh -p -t 1.2 --latest
./copyIIBsToQuay.sh -p -t 1.3 --next
```
...
```
[ERROR] Could not fetch ref_url from https://resultsdb-api.engineering.redhat.com/api/v2.0/results/latest?testcases=cvp.redhat.detailed.operator-catalog-initialization-bundle-image&item=rhdh-operator-bundle-container-1.3-30
```
====
+
. Verify that after moving to 1.3 in main, we have:
.. 1.2 == latest
.. 1.3 == next
...
Once the code freeze is achieved and all the last minute changes have stopped, youll want to:

* disable the 1.y pipeline so you don't get any unexpected/unapproved changes in the release candidate
* ensure you've run `copyIIBsToQuay.sh` to update the IIBs for the RC
* copy the latest NVRs into the errata
* move the errata to `QE` and trigger a push to stage
* collect the NVRs and quay images into an email and send it to QE with a subject like `RHDH 1.2.2 RC-07-24 is ready for QE`
* copy the contents of that email into slack #team-rhdh channel
...
./build/helm/prepare.sh --chart-version "${CHART_VERSION}"" --rhdh-version "${RHDH_VERSION}" --catalog git@github.com:rhdh-bot/openshift-helm-charts.git --publish

# the generated branch will be invalid for creating a PR, as it will be waaay out of date compared to the openshift main branch
cd /tmp
git clone git@github.com:openshift-helm-charts/charts.git --depth 1 && cd charts
git remote add rhdh-bot git@github.com:rhdh-bot/openshift-helm-charts.git && git fetch rhdh-bot && git checkout release-${CHART_VERSION}

# create a pull request from the release-${CHART_VERSION} branch of rhdh-bot to the openshift main branch
...
./build/scripts/copyIIBsToQuay.sh -t 1.1-122 --force --push --latest -e 1.1.1.GA
```
...
./build/scripts/tagRelease.sh -v 1.1.1 -t 1.1 -gh 1.1.x -pd rhdh-1.1-rhel-9 -ghtoken $GITHUB_TOKEN
...
... [ ] install from `fast` - see link:https://github.com/janus-idp/operator/blob/main/.rhdh/docs/installing-ci-builds.adoc[installation]
... [ ] switch to `fast-1.1` or `fast-1.2` subscription channel
... [ ] verify update from of 1.1.0 -> 1.1.1 (or 1.2.0)
