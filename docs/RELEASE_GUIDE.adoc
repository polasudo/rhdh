...
| **3. Build & Test** | Trigger Tekton Pipelines for builds and Functional Bundle Containers (FBCs). Run automated tests to ensure stability. Review Tekton pipeline status: link:../.tekton/README.adoc[Tekton Pipelines]. | QE, Engineering
...
**Update the Catalog Sources (IIBs) using the https://gitlab.cee.redhat.com/rhidp/rhdh/-/blob/rhdh-1-rhel-9/build/scripts/renderCatalogs.sh?ref_type=heads[renderCatalogs.sh] script:**
```/home/fndlovu/pp/renderCatalogs.sh
```
If replacements are incorrect, modify `catalog-template.json` and rerun the script. | Release Engineering, PM, QE
...
```
Notify QE for final validation. | Docs, PM, QE

| **6. GA Release** | **Trigger the Helm Chart release:**
...
| **7. Post-Release** | **Remove EOL versions** from Comet:
Delete old versions: link:https://comet.engineering.redhat.com/containers/repositories?view=myRepos&filter=repoPath:rhdh[Comet].
Monitor Quay image updates.
Update pipeline runner images **at least once per quarter**. | Security, Release Engineering
|===
