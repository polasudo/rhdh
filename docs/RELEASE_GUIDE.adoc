...
. **FOR 1.4 and earlier** Create new gitlab pipeline schedule for stable branch 1.4 (to run in parallel with CI builds from 1.next)
...
For 1.3 and earlier, run the copy to quay script:

```
./copyIIBsToQuay.sh -p -t 1.3 --latest # until we have a stable branch for 1.4, latest = 1.3
# OR 
./copyIIBsToQuay.sh -p -t 1.2
```

For 1.4+, run the link:../build/scripts/renderCatalogs.sh[renderCatalogs] script:

```
./build/scripts/renderCatalogs.sh -v 1.5.0 --next
# OR
./build/scripts/renderCatalogs.sh -v 1.4.0 --latest
```
