...
+
```
cd /tmp; git clone git@gitgit@gitlab.cee.redhat.com:rhidp/rhdh.git --depth 1 && cd rhdh

# use tag from https://quay.io/repository/rhdh/rhdh-hub-rhel9?tab=tags
./build/helm/prepare.sh --chart-version 1.1.1 --rhdh-version 1.1-102 --catalog git@github.com:rhdh-bot/openshift-helm-charts.git --publish
```
