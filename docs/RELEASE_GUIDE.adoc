...
Run the branch creation script (link:../build/scripts/tagRelease.sh[tagRelease.sh]) whenever dev is done (or nearly done) and it's time for feature (or code) freeze.
...
If so, be sure to add a new (or remove an EOL) FBC application from link:https://gitlab.cee.redhat.com/releng/konflux-release-data/-/tree/main/tenants-config/cluster/stone-prod-p02/tenants/rhdh-tenant/fbc[?ref_type=heads]rhdh-tenant/fbc]. Once merged you should see changes under link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/applications[application], link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/applications/fbc-4-20/components[component] and link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/release[RP] + link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/release/release-plan-admission[RPA]. These are all controlled from the  https://gitlab.cee.redhat.com/releng/konflux-release-data[konflux-release-data Pipelines As Code repo]. When a 1.y stream goes end of life, you can remove old RPs, RPAs, and the application and its components. 
...
----
# Create '1.y' branches 
# - 'release-1.7' branch (https://github.com/redhat-developer/rhdh/tree/release-1.7)
# - 'rhdh-1.7-rhel-9' branch (https://gitlab.cee.redhat.com/rhidp/rhdh/-/tree/rhdh-1.7-rhel-9)
#
# then update 
...
... **NOTE**: wait for generated PR to be refreshed by rhdh-operator GH actions before merging! If your PR changes, then there's a bug in the tagRelease.sh which is causing changes that the rhdh-operator repo doesn't wnt. Review the difference and update tagRelease.sh accordingly.
...
.. link:https://gitlab.cee.redhat.com/releng/konflux-release-data[Konflux release data] (to add new link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/release/release-plan-admission[RPAs], link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/release[RPs], link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/applications[Applications], and link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/applications/rhdh-1-7/components[Components] (including new plugin catalog builders and plugins).

NOTE: **[1.7+]** link:../build/scripts/tagRelease.sh[tagRelease.sh] should generate MRs for all three repos mentioned above, including adding new plugins with: link:https://gitlab.cee.redhat.com/rhidp/rhdh-plugin-catalog/-/blob/rhdh-1-rhel-9/build/scripts/generatePyxisConfigForPlugins.sh[generatePyxisConfigForPlugins.sh] and link:https://gitlab.cee.redhat.com/rhidp/rhdh-plugin-catalog/-/blob/rhdh-1-rhel-9/build/scripts/generateKonfluxReleaseDataForPlugins.sh[generateKonfluxReleaseDataForPlugins.sh]. 
...
* https://gitlab.cee.redhat.com/releng/konflux-release-data/-/merge_requests/8157 (app, components, RPs, RPAs for RHDH) - requires link:https://gitlab.cee.redhat.com/prodsec/product-definitions/-/merge_requests/3720[MR 3720] be merged first
* https://gitlab.cee.redhat.com/releng/konflux-release-data/-/merge_requests/8158 (app, component, RPs, RPAs, for plugin catalog) - requires link:https://gitlab.cee.redhat.com/prodsec/product-definitions/-/merge_requests/3720[MR 3720] be merged first
resolve link:https://issues.redhat.com/browse/RHIDP-7874[RHIDP-7874]
...
As the 'rhdh-bot@redhat.com' user, go to link:https://gitlab.cee.redhat.com/rhidp/rhdh/-/pipeline_schedules[pipeline schedules] and create new pipeline for the `1.y` branch, then point it at the recently created `rhdh-1.y-rhel-9` branch. 
...
* https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/applications/rhdh-1-7/activity (for `1.7.z` builds)
...
. After moving to 1.8 in main and rebuilding from the 1.next application in konlfux, verify the following tags in the Quay repositories have moved:
* `1.6` == `1.6`;  `1.7` == `latest`;  `1.8` == `next
...
.. operator install by using https://github.com/redhat-developer/rhdh-operator/blob/main/.rhdh/scripts/install-rhdh-catalog-source.sh for `-v 1.7` or `--latest` or `--next`
+
[WARNING]
====
...
# NOTE: '-sb' set scripts branch to 'rhdh-1-rhel-9' to use the latest scripts.
/path/to/gl/rhidp/rhdh/build/scripts/updateBaseImages.sh -b release-1.7 -sb rhdh-1-rhel-9 -w $(pwd) -f \*ockerfile -maxdepth 5 --sha

# choose the correct branch
git clone -b release-1.y git@github.com:redhat-developer/rhdh-operator.git
cd rhdh-operator

# NOTE: [rhdh-operator] renovate bot includes SHA digest suffix. Therefore, we need to pass '--sha' option to the script to include it in the image tag. This is to avoid unnecessary updates.
...
** This step is to avoid confusion between "latest build" and "the build you want for the RC"; you can still use `--snapshot` flags to select an older link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/applications/rhdh-1-7/snapshots[*snapshot*], but this is cleaner and safer.
...
** find the correct pipeline, eg., '1.7: Sync upstream...'' 
...
./build/scripts/triggerRespin.sh -v 1.7 hub,op 
or
# rebuild in Konflux only
./build/scripts/triggerRespin.sh -v 1.7 hub -k 
```
+
[[bash-requirement-note]]
[NOTE]
====
[🍎 macOS_arm64]
`triggerRespin.sh`,`sync-midstream.sh`, `kfuxRelease.sh` and other scripts require GNU bash 5.0 or later. If you're using an older version or the default macOS bash, please refer to <<prepare-os-arm64, macOS preparation note>> to install GNU bash and skip `install bash` part:

. Install GNU bash using Homebrew:
+
```
brew install bash
```
. Add the Homebrew bash to your PATH by adding this line to your shell profile (e.g., `~/.zshrc` or `~/.bash_profile`):
+
```
export PATH="/opt/homebrew/bin:$PATH"
```
. Update the script to use the new bash by running:
+
```
sed -i 's|/bin/bash|/usr/bin/env bash|' ./build/scripts/triggerRespin.sh
sed -i 's|/bin/bash|/usr/bin/env bash|' ./build/scripts/sync-midstream.sh
```
Alternatively, you can manually edit script files by changing the shebang line from `\#!/bin/bash` to `#!/usr/bin/env bash` at the top.
====
...
+
. To create a new operator-bundle, do this (latest or next flag will be computed automatically):
+
```
# choose the correct branch
git checkout rhdh-1.7-rhel-9
./build/scripts/triggerRespin.sh -v 1.7 bun
...
====
+
. Publish new FBCs to link:https://quay.io/repository/rhdh/iib[quay.io/rhdh/iib] by using link:../build/scripts/renderCatalogs.sh[renderCatalogs.sh] script. *Run the script from the correct `rhdh-1.y-rhel-9` branch*, and it will provide usage intructions you can then copy into the console. Latest or next flag will be added automatically if appropriate. Rendering, committing changes, and running builds for each OCP version should take no more than 20-30 mins.
...
====
[🍎 macOS_arm64]
`checkImagesInIIB.sh` requires GNU tar. If you're using the default BSD tar, please refer to <<prepare-os-arm64, macOS preparation note>>
====
+
```
RHDH_VERSION=1.7
...
./build/scripts/kfuxRelease.sh --stage -v 1.7.1 -c rhdh-operator-bundle:1.7-51 \
    --cve ~/tmp/cve_csv_exports/1.7/RHDH\ CVE\ Management\ -\ 1.7.1.csv  \
    --debug \
...
*** For example: ./build/scripts/sendSlackNotifications.sh --version 1.7.1 --bundle-tag 1.7-151 --slack-webhook <slack-webhook-url> --release-state RC [optional: --dryrun]
**** [🍎 macOS_arm64]
...
. **Remind PM** to update https://developers.redhat.com/rhdh/plugins with latest plugin lists and link to the docs (https://docs.redhat.com/en/documentation/red_hat_developer_hub/1.7)
...
. Export the link:https://docs.google.com/spreadsheets/d/1JZVTc03wirx-bTpjn3muWed8cGyFTRTNe6x3Gr02hys/edit?gid=973768917#gid=973768917[RHDH CVE Management tracker] from the correct sheet for the release you're about to do (eg., 1.7.1) using File > Download > Comma Separated Values (.csv). Save to a file such as `"/tmp/RHDH CVE Management - 1.7.1.csv"`
...
# use the CVE list file you exported from the RHDH CVE Management tracker above
# with list of JIRAs to close (recommended), and snapshot override (if needed)
./build/scripts/kfuxRelease.sh --prod -v 1.7.1 -c rhdh-operator-bundle:1.7-51 \
    --cve ~/tmp/cve_csv_exports/1.7/RHDH\ CVE\ Management\ -\ 1.7.1.csv  \
    --issues RHIDP-6469,RHIDP-6937,RHIDP-6470 \
    --snapshot rhdh-1-7-8qdgl
```
. Monitor container image release using links provided by link:../build/scripts/kfuxRelease.sh[kfuxRelease.sh]. Release can be seen from link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/applications/rhdh-1-7/releases/
[rhdh-1-7/releases] and should be done in 15-20 mins, depending on network traffic. 
...
====
[NOTE]
The `--latest` flag is ONLY for the latest stable release, not for maintenance. For example if `main` branch is currently at version `1.8.0`, then the `release-1.7` branch (eg., `1.7.0`) is the "latest" branch. If releasing a `.z` update from an older maintenance stream, DO NOT use `--latest`. This alignment changes every quarter when a new `release-1.y` branch is created, aligning to a NEW set of `latest` tags. 
====
...
. Monitor OCP EOL dates - delete old FBC Applications from Konflux to stop building for that OCP version - see link:RHDH-Konflux-overview-onboarding.adoc[RHDH-Konflux-overview-onboarding] (TODO: delete link:https://konflux-ui.apps.stone-prod-p02.hjvn.p1.openshiftapps.com/ns/rhdh-tenant/applications/fbc-4-15[4.15 FBC application] (EOL))
...
```
This script creates link:https://github.com/redhat-developer/rhdh/branches/all?query=release-1.7[release-1.7] and link:https://gitlab.cee.redhat.com/rhidp/rhdh/-/tree/rhdh-1.7-rhel-9[rhdh-1.7-rhel-9] branches and updates `main`.
...
```./build/scripts/kfuxRelease.sh --prod -c rhdh-operator-bundle:1.7-zzz -v 1.7.1 --cve /tmp/RHDH\ CVE\ Management\ -\ 1.7.1.csv
```

**Helm Chart PR generation:**

```./build/helm/prepare.sh --chart-version 1.7.1 --rhdh-version 1.7.1 --chart-branch release-1.7 --publish
```
...
```./build/scripts/kfuxRelease.sh --prod --fbc :1.7.1 [--debug] [--auto]
```

**Repository tagging & updates:**

```export GITHUB_TOKEN=rhdh-bot-token-goes-here
./build/scripts/tagRelease.sh -v 1.7.1 -t 1.7 -gh release-1.7 --midstream-branch rhdh-1.7-rhel-9 --clean --force-update --nobuild
``` | Release Engineering
