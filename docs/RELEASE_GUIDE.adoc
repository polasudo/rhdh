...
### Create branches (eg., 1.4.x, 1.5.x)

Use the link:../build/scripts/tagRelease.sh[tagRelease.sh] script like this:
```
# to create 1.4.x + rhdh-1.4-rhel-9 branches, then update main + rhdh-1-rhel-9 branches
export GITHUB_TOKEN=<your_GH_token>
./build/scripts/tagRelease.sh -t 1.4 --branchfrom main -gh 1.4.x
```
...
. Create new gitlab pipeline schedule for stable branch 1.4 (to run in parallel with CI builds from 1.x)
...
* collect the NVRs and quay images into an email and send it to QE with a subject like `RHDH 1.4.2 RC-07-24 is ready for QE`
...
# create pull request
./build/helm/prepare.sh --chart-version 1.4.2 --rhdh-version 1.4-132 --chart-branch 1.4.x --catalog git@github.com:rhdh-bot/openshift-helm-charts.git --publish
```
...
./build/scripts/tagRelease.sh -v 1.4.2 -t 1.4 -gh 1.4.x -pd rhdh-1.4-rhel-9 -ghtoken $GITHUB_TOKEN
