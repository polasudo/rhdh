...
See link:RELEASE_GUIDE_plugin_catalog.adoc[RELEASE_GUIDE_plugin_catalog.adoc]
...
# NOTE: use the '-sb' flag to set scripts branch to 'rhdh-1-rhel-9' for latest scripts.
...
+
. As of 1.7, GL pipelines will link:../build/ci/sync-midstream.sh#L663[automatically respin] the link:../build/ci/update-bundle-and-FBCs.sh[bundle and FBC builds] if a new hub or operator image is found in quay.io/rhdh. 
.. To manually create a new operator-bundle, do this (latest or next flag will be computed automatically):
+
```
# choose the correct branch
git checkout rhdh-1.7-rhel-9
