...
var timer_services = require('./timer.services.cjs.js');
...
class ArgoService {
  ...
  async deleteAppandProject({
    ...
    if (terminateOperation) {
      ...
      if (terminateOperationResp.statusCode !== 404 && terminateOperationResp.statusCode !== 200 && "message" in terminateOperationResp) {
    ...
    if (deleteAppResp.statusCode !== 404 && deleteAppResp.statusCode !== 200 && "message" in deleteAppResp) {
      deleteAppDetails = {
        status: "failed",
      ...
      for (let attempts = 0; attempts < configuredWaitCycles; attempts++) {
        ...
        if (applicationInfo.statusCode !== 404 && "message" in applicationInfo) {
          ...
          if (attempts < configuredWaitCycles - 1)
            await timer_services.timer(configuredWaitInterval);
        }
      }
    }
    if (continueToDeleteProject) {
      ...
      if (deleteProjectResponse.statusCode !== 404 && deleteProjectResponse.statusCode !== 200 && "message" in deleteProjectResponse) {
...
exports.ArgoService = ArgoService;
//# sourceMappingURL=argocd.service.cjs.js.map
