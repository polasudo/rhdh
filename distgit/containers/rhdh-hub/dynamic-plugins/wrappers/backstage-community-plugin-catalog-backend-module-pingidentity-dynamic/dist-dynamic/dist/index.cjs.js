...
var require$$0$3 = require('uuid');
var require$$0$1 = require('@backstage/catalog-model');
var require$$0$2 = require('node-fetch');

var index_cjs = {};

var catalogModulePingIdentityEntityProvider_cjs = {};

var PingIdentityEntityProvider_cjs = {};
...
constants_cjs.PING_IDENTITY_DEFAULT_ENTITY_QUERY_SIZE = PING_IDENTITY_DEFAULT_ENTITY_QUERY_SIZE;
constants_cjs.PING_IDENTITY_ID_ANNOTATION = PING_IDENTITY_ID_ANNOTATION;
...
const findGroupMemberships = (userId, groups, groupMembersMap) => {
  ...
  groups.forEach((group) => {
    const groupId = group.metadata.annotations ? group.metadata.annotations[constants$1.PING_IDENTITY_ID_ANNOTATION] : void 0;
      ...
      return await transformer(
        {
          ...
          metadata: {
            name: user.username,
            annotations: {
              [constants$1.PING_IDENTITY_ID_ANNOTATION]: user.id,
      ...
      return await transformer(
        {
          ...
          metadata: {
            name: group.name,
            annotations: {
              [constants$1.PING_IDENTITY_ID_ANNOTATION]: group.id,
...
const readPingIdentity = async (client, options) => {
  ...
  groups.forEach((group) => {
    const groupId = group.metadata.annotations[constants$1.PING_IDENTITY_ID_ANNOTATION];
    groupsMap.set(groupId, group);
  });
  groups.forEach((group) => {
    const parentGroupId = parentGroupMap.get(
      group.metadata.annotations[constants$1.PING_IDENTITY_ID_ANNOTATION]
    );
    if (parentGroupId) {
...
read_cjs.readPingIdentity = readPingIdentity;
...
client_cjs.PingIdentityClient = PingIdentityClient;

var uuid = require$$0$3;
...
class PingIdentityEntityProvider$1 {
  constructor(options) {
  ...
  static fromConfig(configRoot, options) {
    return config.readProviderConfigs(configRoot).map((providerConfig) => {
      ...
      const provider = new PingIdentityEntityProvider$1({
      ...
      await taskRunner.run({
        ...
        fn: async () => {
          const logger = this.options.logger.child({
            class: PingIdentityEntityProvider$1.prototype.constructor.name,
            taskId: id,
...
PingIdentityEntityProvider_cjs.PingIdentityEntityProvider = PingIdentityEntityProvider$1;

var extensions_cjs = {};

var backendPluginApi$1 = require$$0;

const pingIdentityTransformerExtensionPoint = backendPluginApi$1.createExtensionPoint({
  id: "pingIdentity.transformer"
});

extensions_cjs.pingIdentityTransformerExtensionPoint = pingIdentityTransformerExtensionPoint;

var backendPluginApi = require$$0;
var alpha = require$$1;
var PingIdentityEntityProvider = PingIdentityEntityProvider_cjs;
var extensions = extensions_cjs;

const catalogModulePingIdentityEntityProvider$1 = backendPluginApi.createBackendModule({
  pluginId: "catalog",
...
catalogModulePingIdentityEntityProvider_cjs.catalogModulePingIdentityEntityProvider = catalogModulePingIdentityEntityProvider$1;

Object.defineProperty(index_cjs, '__esModule', { value: true });

var catalogModulePingIdentityEntityProvider = catalogModulePingIdentityEntityProvider_cjs;



var _default = index_cjs.default = catalogModulePingIdentityEntityProvider.catalogModulePingIdentityEntityProvider;
