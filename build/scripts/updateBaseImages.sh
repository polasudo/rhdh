	...
	if [[ -f ${d} ]]; then
		...
		for URL in $URLs; do
			...
			if [[ $URL == "https"* ]]; then 
				...
				URL_FULL="${URL}"
				GLIT_REPOFLAG=""
				FROMPREFIX_GLIT_TAG=${FROMPREFIX##*:}
				if [[ $FROMPREFIX_GLIT_TAG != "$FROMPREFIX" ]]; then
					GLIT_TAG="$FROMPREFIX_GLIT_TAG"
				...
				if [[ -x ${SCRIPT_DIR}/getLatestImageTags.sh ]]; then
					...
					if [[ $VERBOSE -eq 1 ]]; then echo "Downloading getLatestImageTags.sh"; fi
					pushd /tmp >/dev/null || exit
					curl -sSLO "https://gitlab.cee.redhat.com/rhidp/rhdh/-/raw/${SCRIPTS_BRANCH}/build/scripts/getLatestImageTags.sh" && chmod +x getLatestImageTags.sh
				...
				if [[ ${LATESTTAG} ]]; then
					...
					CURR_TAGver=${URL%@sha256:*}; CURR_TAGver=${CURR_TAGver##*:}; CURR_TAGver=${CURR_TAGver%%-*} # 1.0
					...
					if [[ "${LATE_TAGver}" != "${CURR_TAGver}" ]] || [[ ${LATE_TAGrevbase} -gt ${CURR_TAGrevbase} ]] || [[ ${LATE_TAGrevsuf} -gt ${CURR_TAGrevsuf} ]]; then
						if [[ $LATE_TAGver != "???" ]] && [[ $LATE_TAGrev != *"container. Is the container public and populated"* ]]; then
							...
							if [[ "${testvercomp_return}" == "true" ]] || [[ ${LATE_TAGrevsuf} -ge ${CURR_TAGrevsuf} ]] || [[ ${LATE_TAGrevbase} -gt ${CURR_TAGrevbase} ]]; then # fix the ${DOCKERFILE}
								...
								LATESTTAGSHA=""
								if [[ $USE_DIGESTS -eq 1 ]]; then
									# append the digest on the tag
									REGISTRYPRE="${URL_FULL##*://}"; REGISTRYPRE="${REGISTRYPRE%:*}"
									LATESTTAGSHA="@$(skopeo inspect docker://${REGISTRYPRE}:${LATESTTAG} |jq -r '.Digest' || echo "")"
									if [[ $VERBOSE -eq 1 ]]; then echo "skopeo inspect docker://${REGISTRYPRE}:${LATESTTAG} --> .Digest = $LATESTTAGSHA"; fi
								fi
