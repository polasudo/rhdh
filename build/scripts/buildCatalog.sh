...
USEKANIKO=0 # if 1, use environment configured from https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/blob/rhdh-1.0-rhel-9/build/dockerfiles/kaniko-ubi9.Dockerfile
AUTHFILEFLAGS=""
VERBOSE=0
WORKING_DIR='./'
...
while [[ "$#" -gt 0 ]]; do
  case $1 in
    ...
    '--kaniko') USEKANIKO=1;;
    '--destination') DESTINATIONFLAGS="${DESTINATIONFLAGS} $1 $2"; shift 1;;
    '--authfile') AUTHFILEFLAGS="$1 $2"; shift 1;;
    '-v'|'--verbose') VERBOSE=1;;
    '-h'|'--help') usage;;
...
# sudo the podman commands
if [[ $SUDO -eq 1 ]]; then PODMAN="sudo $PODMAN"; fi
  ...
  if [[ "$PUSH" == "true" ]]; then
    for image in $targetIndexImage $DESTINATIONFLAGS; do
      if [[ "$image" != "--destination" ]]; then
        echo -n "[IMG] Push $image ... "
        $PODMAN push $image -q ${AUTHFILEFLAGS} || echo " failed!"
      fi
    done
  fi
else
  # build, including extra destinations (tags)
  # uses environment configured from https://gitlab.cee.redhat.com/rhidp/cpaas-rhdh-hub/-/blob/rhdh-1.0-rhel-9/build/dockerfiles/kaniko-ubi9.Dockerfile
