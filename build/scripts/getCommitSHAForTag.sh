...
# 
# Eg., for quay.io/rhdh/hub-rhel9:1.0-zzz get https://pkgs.devel.redhat.com/cgit/containers/rhdhhub/commit?id=53306c3f99d3b35d4bdeb22b5ef2081e322db7f8
...
# DWNSTM_BRANCH="rhdh${JOB_BRANCH}-rhel-9"
NUMTAGS=1 # by default show only the latest tag for each container; or show n latest ones
CONTAINERS=""
while [[ "$#" -gt 0 ]]; do
  case $1 in
    '-j') JOB_BRANCH="$2"; DWNSTM_BRANCH="rhdh-${JOB_BRANCH}-rhel-9"; shift 2;; 
...
if [[ -z ${candidateTag} ]] && [[ ${DWNSTM_BRANCH} ]]; then
	...
	candidateTag="rhdh-${BASETAG}-rhel-9-container-candidate"
else
...
for d in $CONTAINERS; do
	echo
	echo "$d"
	d=${d/rhdh-rhel9-/} # special case for operator and bundle images
	...
	if [[ ! $candidateTag ]]; then
		...
		candidateTagUsed="rhdh-${TAG%%-*}-rhel-9-container-candidate"
	else
