...
curl -sSLO https://raw.githubusercontent.com/rhdh-bot/openshift-helm-charts/redhat-developer-hub-${CHART_VERSION}/installation/install.sh && chmod +x install.sh
./install.sh ${CHART_VERSION} --namespace rhdh-${CHART_VERSION_OCP} --create-repo
```
...
```
oc apply -f https://github.com/rhdh-bot/openshift-helm-charts/raw/redhat-developer-hub-${CHART_VERSION}/installation/rhdh-next-ci-repo.yaml
```
2. Go to `Developer` perspective in your cluster
1. Select your namespace or project (eg., `rhdh-helm` or `rhdh-${CHART_VERSION_OCP}`)
...
podman run --rm -i -e KUBECONFIG=/.kube/config \\
  ...
  verify --write-to-file https://github.com/rhdh-bot/openshift-helm-charts/raw/redhat-developer-hub-${CHART_VERSION}/charts/redhat/redhat/redhat-developer-hub/${CHART_VERSION}/redhat-developer-hub-${CHART_VERSION}.tgz
